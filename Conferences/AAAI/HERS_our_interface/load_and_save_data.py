...
def save_data_dict_zip(URM_DICT, UCM_DICT, ICM_DICT, splitted_data_path, file_name_prefix):
    dataIO = DataIO(folder_path=splitted_data_path)

    URM_DICT["__UCM_available"] = len(UCM_DICT) > 0
    ...
    if len(UCM_DICT) > 0:
        dataIO.save_data(data_dict_to_save=UCM_DICT, file_name=file_name_prefix + "UCM_dict")
    if len(ICM_DICT) > 0:
